...
// Per-backend health pill (consumes the previously-unused backends store).
const statusByBackend = computed<Record<string, string>>(() => {
  const m: Record<string, string> = {};
  for (const b of backends.backends) m[b.backend_id] = b.status;
  return m;
});
</script>
  ...
  <aside class="sidebar" :class="{collapsed: view.sidebarCollapsed}">
    ...
    <template v-for="g in groups" :key="g.backend">
      <div class="group-label">
        <span>{{ t(`publishers.${g.backend}`) }}</span>
        <i
          v-if="statusByBackend[g.backend]"
          class="group-status-dot"
          :class="statusByBackend[g.backend]"
          :title="t(`backend_status.${statusByBackend[g.backend]}`)"
        ></i>
      </div>
