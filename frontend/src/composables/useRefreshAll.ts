...
// installed game for updates, and clears the news + gacha caches so those
// panels refetch lazily on next view. Shared by the Topbar refresh button and
// the settings panel's post-Save refresh so they cannot drift.
export async function refreshAll(): Promise<void> {
  ...
  gacha.reset(); // same: gacha board reloads from store on next view (no network here)
  await games.load();
